...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			tx, _ := types.SignTx(types.NewTransaction(nonce, userAddr1, "eth", big.NewInt(10_000_000_000_000_000), params.TxGas, big.NewInt(params.InitialBaseFee), nil), signer, bankKey)
